...
.PHONY: docker-build
docker-build:
	DOCKER_BUILDKIT=1 $(DOCKER) build --progress=plain --build-arg PLATFORM_TYPE="$(PLATFORM_TYPE)" $(BUILD_ARGUMENTS) --build-arg bundle_gems__contribsys__com=$(BUNDLE_GEMS__CONTRIBSYS__COM) -t $(DOCKER_DEST) .
