describe Travis::Scheduler::Service::Ping do
  ...
  let!(:one)     { FactoryBot.create(:job, state: :created, owner_id: 1, owner_type: 'User', created_at: now - interval) }
  let!(:two)     { FactoryBot.create(:job, state: :created, owner_id: 1, owner_type: 'User', created_at: now - interval) }
  let!(:three)   { FactoryBot.create(:job, state: :created, owner_id: 2, owner_type: 'User', created_at: now - interval) }
  let!(:four)    { FactoryBot.create(:job, state: :created, owner_id: 3, owner_type: 'user', created_at: now) }
  let!(:five)    { FactoryBot.create(:job, state: :created, owner_id: 3, owner_type: 'Organization', created_at: now - interval) }
