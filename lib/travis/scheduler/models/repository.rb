...
class Repository < ActiveRecord::Base
  ...
  #     without_deleted.where(owner_name: owner_name)
  #   end

  #   def by_member(login)
  #     without_deleted.joins(:users).where(users: { login: login })
  #   end

  #   def by_slug(slug)
  #     without_deleted.where(owner_name: slug.split('/').first, name: slug.split('/').last).order('id DESC')
  #   end
  ...
  #   def search(query)
  #     query = query.gsub('\\', '/')
  #     without_deleted.where("(repositories.owner_name || chr(47) || repositories.name) ILIKE ?", "%#{query}%")
  #   end

  #   def active
  #     without_deleted.where(active: true)
  #   end
  ...
  #     query = %(SELECT owner_name, count(*) FROM repositories WHERE owner_name IN (?) AND deleted_at IS NULL GROUP BY owner_name)
